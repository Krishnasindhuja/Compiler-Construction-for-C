...
# Perl script for testing CSCE 531 project 2 submissions (exprs & fcns)
...
# subdirectories:
#     proj2 -- contains the source files of your submission
#     proj2test -- contains the test files T2L*.c
# The script will run "make" in the proj2 subdirectory, then run the resulting
# executable on files in the proj2test subdirectory, putting the output files
# in the proj2 (source) directory. For each error-free file, it then tries to
...
# file is then compared with the one in the proj2test directory.
...
$project_directory = "/acct/kalusani/compilers/project-2";
$semester = 'su16';
...
$project_installment = 2;
$hw_dir = "proj$project_installment";
$test_suite_dir = "/acct/kalusani/compilers/project-2/proj2test";
...
sub test_compiler {
	...
	foreach $key (sort { $b cmp $a; } (keys(%{$test_file_hash{$level}}))) {
		...
		$sfile = `cat $base.s`;
		# Grab assm code between line numbers 67 and 72
		$sfile =~ /\n #[ \t]*67/s;
		$sfile = $';
		$sfile =~ /\n #[ \t]*72/s;
		$sfile = $` . "\n";
